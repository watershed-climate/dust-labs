...
class FigmaAPI {
  ...
  /**
   * Add a method to listen for selection changes.
   * The callback is invoked whenever a selection change event is received.
  ...
  onSelectionChange(callback: () => void) {
    const listener = (event: MessageEvent) => {
      if (
        event.origin === "https://www.figma.com" &&
        event.data.pluginMessage?.type === "SELECTION_CHANGE"
      ) {
        callback();
      }
    };
