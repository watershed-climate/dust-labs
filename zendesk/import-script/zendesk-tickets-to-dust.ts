...
const TICKETS_UPDATED_SINCE = Math.floor(
...
zendeskApi.interceptors.response.use(
  (response) => {
    ...
    console.log(
      `Endpoint: ${response.config.url}, Rate Limit: ${rateLimit}, Remaining: ${rateLimitRemaining}`
    );
    return response;
  },
  (error) => {
    if (error.response && error.response.status === 429) {
      ...
      console.log(
        `Rate Limit: ${error.response.headers["x-rate-limit"]}, Remaining: ${error.response.headers["x-rate-limit-remaining"]}`
      );
    }
...
async function getTicketsUpdatedLast24Hours(): Promise<number[]> {
  ...
  do {
    try {
      ...
      const response: AxiosResponse<ZendeskIncrementalResponse> =
        await zendeskApi.get("/incremental/tickets.json", {
          params: {
      ...
      const newTickets = response.data.tickets.map(
      ...
      console.log(
        `Page ${currentPage}: Retrieved ${newTickets.length} tickets`
      );
      console.log(
        `Total count: ${totalCount}, Current total: ${
          allTicketIds.length
        }, Next cursor: ${nextPage || "None"}`
      );
...
async function getTicketsBatch(ids: number[]): Promise<Ticket[]> {
  try {
    const response: AxiosResponse<{ tickets: Ticket[] }> = await zendeskApi.get(
...
async function getTicketComments(ticketId: number): Promise<Comment[]> {
  try {
    const response: AxiosResponse<{ comments: Comment[] }> =
      await zendeskApi.get(`/tickets/${ticketId}/comments.json`);
...
async function getUser(userId: number): Promise<User | null> {
  try {
    const response: AxiosResponse<{ user: User }> = await zendeskApi.get(
      `/users/${userId}.json`
    );
...
async function getCustomStatus(statusId: number): Promise<string> {
  try {
    const response: AxiosResponse<{ custom_status: { agent_label: string } }> =
      await zendeskApi.get(`/custom_statuses/${statusId}.json`);
...
${comments
  .map((comment) => {
    ...
    return `
[${comment.created_at}] Author: ${author ? author.name : "Unknown"} (${
...
`;
  ...
  try {
    ...
    console.error(
      `Error upserting ticket ${ticket.id} to Dust datasource:`,
      error
    );
  }
}
...
async function main() {
  try {
    ...
    console.log(
      `Found ${recentTicketIds.length} tickets updated in the last 24 hours.`
    );
      ...
      for (const ticket of tickets) {
        ...
        const assignee =
          ticket.assignee_id !== null
            ? users.get(ticket.assignee_id) || null
            : null;
